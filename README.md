...
![`reins doctor` health check](docs/demo-doctor.png)
...
- **reins is not an OS sandbox.** It's a policy and audit layer. The parser covers the expected technique families — wrapper flags, control-flow keywords, whitespace-variable obfuscation, command substitution and quoted interpreter recursion (the exact corpus lives in the bypass test suite) — but a sufficiently creative agent may still find shapes it doesn't classify. For hard isolation, pair it with bubblewrap/Seatbelt, a microVM (microsandbox), or [Claude Code sandboxing](https://code.claude.com/docs/en/hooks#sandboxing) — they're complements, not substitutes.
