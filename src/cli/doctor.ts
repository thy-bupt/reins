...
export interface DoctorOptions {
  ...
  /** check every adapter without failing on any absence */
  all?: boolean;
}
...
export async function runDoctor(opts: DoctorOptions): Promise<DoctorReport> {
  ...
  const primary = opts.primaryAgent ?? (opts.all ? undefined : "claude");
  if (primary === "claude" || opts.all) {
    const claudeSeverity: "ok" | "warn" | "fail" = primary === "claude" ? "fail" : "warn";
    if (!existsSync(opts.settingsPath)) {
      checks.push({
        name: "claude-hook",
        status: claudeSeverity,
  ...
  // 5. trace integrity + policy drift
  if (existsSync(sessionsDir)) {
    ...
    for (const f of files) {
      ...
      if (!result.ok) {
        broken.push(`${f}: ${result.reason ?? "integrity failure"} at event ${result.brokenAt}`);
        continue;
      }
      // policy drift: more than one distinct policy digest inside one session
      try {
        const events = await readTrace(filePath);
        const digests = new Set(
          events.map((e) => e.policyDigest).filter((d: unknown): d is string => typeof d === "string"),
        );
        if (digests.size > 1) drifted.push(`${f}: ${digests.size} distinct policy digests`);
      } catch {
        broken.push(`${f}: unreadable`);
      }
    }
    const notes = [
      broken.length > 0 ? `${broken.length} tampered/corrupt trace(s): ${broken.join("; ")}` : null,
      drifted.length > 0 ? `policy drift inside ${drifted.length} session(s): ${drifted.join("; ")}` : null,
    ].filter((n): n is string => n !== null);
    if (notes.length === 0) {
      checks.push({ name: "traces", status: "ok", detail: `${files.length} trace(s) verified, hash chains intact` });
    } else if (broken.length > 0) {
      checks.push({ name: "traces", status: "fail", detail: notes.join(" · ") });
    } else {
      checks.push({ name: "traces", status: "warn", detail: notes.join(" · ") });
    }
  } else {
